...
Furthermore, what is great is that in Haskell thinking about evaluation in
terms of substituting equals-for-equals works anywhere. As a result, we could
*also* understand the evaluation of `doTwice` using a more standard order of
evaluation.
...
ex1Test :: Test
ex1Test = ex1 (+ 1) 2 ~?= 6
-- Performs doTwice twice (4 times total)
...
filter pred = foldr (\x y -> if pred x == True then x:y else y) [] 
